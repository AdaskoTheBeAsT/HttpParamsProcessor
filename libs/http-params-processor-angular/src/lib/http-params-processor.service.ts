 ...
 */
export const HTTP_PARAMS_KEY_FORMATTER =
  new InjectionToken<IKeyFormattingStrategy>('HTTP_PARAMS_KEY_FORMATTER');

/**
 * InjectionToken for providing custom value converters application-wide.
 */
export const HTTP_PARAMS_VALUE_CONVERTERS = new InjectionToken<
  IValueConverter[]
>('HTTP_PARAMS_VALUE_CONVERTERS');

/**
 * Type alias for values that can be processed by the HttpParamsProcessorService.
 */
...
export class HttpParamsProcessorService {
  private readonly defaultKeyFormatter: IKeyFormattingStrategy =
    inject(HTTP_PARAMS_KEY_FORMATTER, { optional: true }) ??
    new DefaultKeyFormattingStrategy();

  private readonly defaultValueConverters: IValueConverter[] =
    inject(HTTP_PARAMS_VALUE_CONVERTERS, { optional: true }) ??
  ...
  private createDefaultConverters(): IValueConverter[] {
    return [
      ...
      new ValueConverter(
        ...
        new DefaultPrimitiveValueToStrategy() as IValueToStrategy<
  ...
  processWithParams(
    ...
    const visited = new WeakSet<object>();
    const { keyFormatter, valueConverters } = this.resolveOptions(options);
    return this.processWithParamsInternal(
      params,
      key,
      obj,
      visited,
  ...
  private resolveOptions(options?: HttpParamsProcessorOptions): {
    keyFormatter: IKeyFormattingStrategy;
    valueConverters: IValueConverter[];
  } {
    if (!options) {
  ...
  private processWithParamsInternal(
    ...
    if (
      ...
      const transformed = keyFormatter.transformComplexObject(
        params,
        key,
  ...
  private processObject(
    ...
    for (const property in obj) {
      ...
      retPar = this.processWithParamsInternal(
        retPar,
        name,
        obj[property],
        visited,
